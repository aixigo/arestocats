...
const htmlDir = `${reportsPath}/html`;
...
function report( allResults, jobItems, jobResult ) {
   ...
   allResults.forEach( result => {
      ...
      fs.writeFileSync( path.join( htmlDir, `${result.subject.name}.csv` ), asCsv( testsuites ) );
   } );
   // summarizes all existing csv into one for better overview in jenkins project homepage.
   summarizeCsv( htmlDir );
}
...
function summarizeCsv( dir, output = 'summary.csv' ) {
   // collects csvs from scenario into one summary.
